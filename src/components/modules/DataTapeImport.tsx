...
const ALIASES: Record<string, string> = {
  'referencia':'external_ref','ref':'external_ref','id':'external_ref',
  ...
  'tipo':'property_type','type':'property_type',
...
const NUMERIC = ['gross_area','useful_area','land_area','floor','year_built','fee_amount']
const FIELDS  = ['external_ref','address','parish','municipality','district','postal_code',
  ...
  const importMutation = useMutation({
    mutationFn: async () => {
      ...
      const props = rows.map((row: any, i: number) => {
        const p: any = { portfolio_id: portfolioId, client_id: pf.client_id, ref: `AV-${String(i+1).padStart(4,'0')}`, datatape_data: row }
        Object.entries(mapping).forEach(([col, field]) => {
          const v = row[col]
          if (v !== null && v !== undefined) {
            p[field] = NUMERIC.includes(field) ? parseFloat(String(v)) || null : String(v).trim() || null
          }
        })
        return p
      })
      ...
      await supabase.from('datatape_imports').insert({ portfolio_id: portfolioId, file_name: fileName, row_count: rows.length, imported_count: imported })
      return imported
    },
    onSuccess: (n: number) => {
      qc.invalidateQueries({ queryKey: ['properties'] })
      qc.invalidateQueries({ queryKey: ['dashboard-stats'] })
      toast.success(`${n} imóveis importados`); setStep('done')
    },
      ...
      <div className="p-6 max-w-3xl space-y-6">
          ...
          <div className="space-y-5">
            ...
            <div className="card">
              <h3 className="text-sm font-semibold text-gray-700 mb-4">
                Mapeamento de colunas <span className="font-normal text-gray-400 ml-2">({Object.values(mapping).filter(Boolean).length} auto-detectadas)</span>
              </h3>
              <div className="space-y-2 max-h-80 overflow-y-auto">
                {headers.map(h => (
                  <div key={h} className="flex items-center gap-3">
                    ...
                    {mapping[h] ? <CheckCircle size={14} className="text-emerald-500 flex-shrink-0" /> : <AlertCircle size={14} className="text-gray-300 flex-shrink-0" />}
                  </div>
                ))}
              </div>
            </div>
        ...
        {step === 'done' && (
          <div className="card text-center py-10">
            ...
            <p className="text-sm text-gray-500 mt-1">{rows.length} imóveis adicionados</p>
