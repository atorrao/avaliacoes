...
import { ArrowLeft, Upload, Trash2, FileSpreadsheet, MapPin, Loader2, FileText, ExternalLink, Save } from 'lucide-react'
...
const TAB_LABELS: Record<Tab,string> = {
  ...
  photos: 'Fotos', prev: 'Aval. anterior',
  comps: 'Comparáveis', billing: 'Faturação', notes: 'Notas'
}
const SLOTS = [1,2,3,4,5]

function useLeaflet(ready: boolean, cb: () => void) {
  useEffect(() => {
    if (!ready) return
...
function EditField({ label, value, field, type='text', onSave }: { label:string; value:any; field:string; type?:string; onSave:(patch:any)=>void }) {
      ...
      <div className="flex items-center gap-4 px-6 py-4 border-b border-gray-100 bg-white">
        ...
        <div className="flex-1">
          ...
          <p className="text-sm text-gray-500">
            {[property.street, property.number, property.municipality, property.district].filter(Boolean).join(', ') || property.address || ''}
          </p>
        ...
        {tab === 'info' && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="card space-y-3">
              ...
              <EditField label="Área m² (col. N)"      field="area_m2"         value={property.area_m2}         type="number" onSave={save} />
              <EditField label="Garagem m² (col. O)"   field="area_garage_m2"  value={property.area_garage_m2}  type="number" onSave={save} />
              <EditField label="Anexo m² (col. P)"     field="area_annex_m2"   value={property.area_annex_m2}   type="number" onSave={save} />
            ...
            <div className="card space-y-3">
              <h3 className="text-sm font-semibold text-gray-700 mb-1">Outros dados</h3>
              ...
              <EditSelect label="Estado visita" field="visit_status" value={property.visit_status}
                options={Object.entries(VISIT_STATUS_LABELS)} onSave={save} />
          ...
          <div className="space-y-4">
            <div className="flex items-end gap-3 flex-wrap">
              <div>
                ...
                <input type="number" step="0.000001" className="input w-40"
                  defaultValue={property.latitude||''} key={`lat-${property.latitude}`}
                  onBlur={e => e.target.value && update.mutate({ latitude: parseFloat(e.target.value) })} />
              </div>
              <div>
                <label className="label">Longitude</label>
                <input type="number" step="0.000001" className="input w-40"
                  defaultValue={property.longitude||''} key={`lon-${property.longitude}`}
            ...
            {!property.latitude && (
              <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
                Usa morada + código postal. Se não encontrar, usa o código postal como localização aproximada. O marcador é arrastável para ajustar.
              </p>
            )}
                ...
                {property.prev_valuation_report_path ? (
                  <div className="space-y-2">
                    ...
                    <button className="btn text-xs text-red-500 hover:bg-red-50 w-full"
                      onClick={() => update.mutate({ prev_valuation_report_path: null })}>Remover relatório</button>
                  </div>
                ) : (
        ...
        {tab === 'billing' && (
          <div className="card max-w-lg space-y-4">
            ...
            <EditSelect label="Estado" field="billing_status" value={property.billing_status}
              options={Object.entries(BILLING_STATUS_LABELS)} onSave={save}/>
  ...
  const save = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from('market_comps').insert({
        property_id:propertyId, portal:f.portal, listing_ref:f.listing_ref||null,
        typology:f.typology||null, area_m2:f.area_m2?parseFloat(f.area_m2):null,
        price:f.price?parseFloat(f.price):null, listing_date:f.listing_date||null,
      })
      if (error) throw error
    },
  ...
  return (
    <div className="card">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        ...
        <div><label className="label">Tipologia</label><input className="input" value={f.typology} onChange={e => setF(p => ({...p, typology:e.target.value}))}/></div>
